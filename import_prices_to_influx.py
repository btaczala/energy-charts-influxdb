...
while True:
    ...
    influx_mode = bool(args.token and args.url and args.org)
    if not influx_mode:
        logger.info(
            "InfluxDB credentials not provided. Dumping 5 first and 5 last entries to stdout."
        )
        first_5_timestamps = timestamps[:5]
        first_5_prices = prices[:5]
        last_5_timestamps = timestamps[-5:] if len(timestamps) > 5 else []
        last_5_prices = prices[-5:] if len(prices) > 5 else []
        for ts, price in zip(first_5_timestamps, first_5_prices):
            print(
                f"{datetime.datetime.now().isoformat()} ({time.time()}): {datetime.datetime.fromtimestamp(ts.timestamp())}: {price}"
            )
        if last_5_timestamps:
            print("...")  # Separator for last 5
            for ts, price in zip(last_5_timestamps, last_5_prices):
                print(
                    f"{datetime.datetime.now().isoformat()} ({time.time()}): {datetime.datetime.fromtimestamp(ts.timestamp())}: {price}"
                )
        if not args.loop:
